...
open(GEN_JL, "w") do io
    ...
    ### From generated.toml
    alldata = open(Pkg.TOML.parse, GEN_TOML)

    # First pass to add extra functions
    for (name, data) in copy(alldata)
        if haskey(data, "caller")
            get!(Dict, alldata, data["caller"])["calls"] = name
        end
        ...
        if haskey(data, "errchecker")
            get!(Dict, alldata, data["errchecker"])["errchecks"] = name
        end
    end

    # Second pass to generate functions
    for (name, data) in alldata
        ...
        if cobj !== nothing
            ...
            elseif haskey(cobj, "cobjptr")
                ex = """unsafe_load(cglobal((:$(cobj["cobjptr"]), PYLIB), PyPtr))"""
            else
        ...
        calls = get(data, "calls", nothing)
        if calls !== nothing
            println(io, "$uname(args...; kwargs...) = unsafe_pycall_args($calls(), args, kwargs)")
        end

        # type check
        checks = get(data, "typechecks", nothing)
        if checks !== nothing
            println(io, "$name(o) = unsafe_pytype_check(o, $checks())")
        end
        ...
        # error check
        checks = get(data, "errchecks", nothing)
        if checks !== nothing
            println(io, "$name() = pyerror_occurred($checks())")
        end

        # error set
        sets = get(data, "errsets", nothing)
        if sets !== nothing
            println(io, "$name() = pyerror_set($sets())")
            println(io, "$name(arg) = pyerror_set($sets(), arg)")
        end
